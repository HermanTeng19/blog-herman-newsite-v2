...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <div className="project-card bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200 h-full flex flex-col">
      <Image
        src={project.image}
        alt={project.title}
        width={600}
            ...
            <div className="flex gap-2">
              {project.link && (
                <Link
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-300 text-center"
                >
                  View Live Site
                </Link>
              )}
              {project.github && (
                <Link
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`${project.link ? 'flex-1' : 'w-full'} bg-gray-800 hover:bg-gray-900 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-300 flex items-center justify-center gap-2`}
                >
